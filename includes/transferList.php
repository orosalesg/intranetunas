...
if ($wareHouse == "" || $wareHouse == NULL || $wareHouse == 0) {
	...
	$queryTotal.= " WHERE dsStore = '$wareHouse'";
}
...
while($row = $result->fetch(PDO::FETCH_ASSOC)) {
	...
	$nestedData[] = utf8_encode($row["orStore"]);
	$nestedData[] = utf8_encode($row["dsStore"]);
	$nestedData[] = utf8_encode($row["code"]);
	$nestedData[] = utf8_encode($row["created"]);
	$nestedData[] = utf8_encode($row["emp"]);
	$nestedData[] = "<a href='transferDetail.php?tranID=".$row["ID"]."'><i class='fa fa-eye' aria-hidden='true'></i></a>";
	$data[] = $nestedData;	
}
