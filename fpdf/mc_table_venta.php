...
function Header()
{
	...
	$query = "SELECT T1.ID, T1.code, T2.ID storeID, T2.name store, CONCAT(T3.first, ' ', T3.last) emp, DATE_FORMAT(T1.created_at, '%d-%m-%Y %T') created_at, T1.remarks FROM SALES T1 INNER JOIN STORES T2 ON T1.storeID = T2.ID INNER JOIN CREW T3 ON T1.empID = T3.ID WHERE T1.ID = '$salID'";
	...
	$queryTo = "SELECT T1.first, T1.last, T1.email, T2.phone, T2.address FROM CREW T1 JOIN STORES T2 ON T1.storeID = T2.ID WHERE T2.ID = '$storeID' LIMIT 1";
    ...
    // Title
	$this->Cell(98,8,'',0,0,'');
	$this->Cell(98,8,$store,0,1,'R');
	$this->SetFont('Arial','B',12);
	$this->Cell(98,5,'',0,0,'');
	$this->Cell(98,5,utf8_decode('Venta ').$folio,0,1,'R');
	$this->Cell(196,5,'',0,1,'L');
	$this->SetFont('Arial','B',8);
	$this->Cell(49,5,'Fecha de venta',0,0,'L');
	$this->Cell(49,5,utf8_decode('Salón de venta'),0,0,'L');
	$this->Cell(49,5,'Nombre',0,0,'L');
	$this->Cell(49,5,'E-mail',0,1,'L');
	$this->SetFont('Arial','',8);
	$this->Cell(49,5,$created_at,0,0,'L');
	$this->Cell(49,5,$store,0,0,'L');
	...
	$this->SetWidths(array(16,40,90,25,25));
	$this->Row(array("Cantidad",utf8_decode("Código"),"Producto","Precio","Importe"));
